...
//		cfg.flashplayer = [];
	...
	appendvideo(html, num) {
		...
		cfg.flashinfo = [];
		for(var i = 0; i < num; i++) {
			html += '<div class="list"  id=list_' + i + '  ><div class="title">' + cfg.channeltitle[i] + '</div><div style="width:100%;height:100%"    id=video_' + i + ' ></div></div>';
			cfg.flashinfo.push({
				num: i,
				id: 'video_' + i,
				url: cfg.channel[i]
	 ...
	 * 
	 * */
    resetflashinfo (urlarr) {
      for(let j = 0; j < cfg.channelnum; j++) {
		cfg.flashinfo[j].url = urlarr[j];
	  }
      this.setckplayer()
    }
	/*
	 ...
	 */
	ckplayer(obj) {
		let callback = 'loadedHandler' + obj.num;
		var flashvars = {
			f: obj.url, //视频地址
		...
		CKobject.embedSWF('ckplayer/ckplayer.swf', obj.id, 'ckplayer_' + obj.id, '100%', '100%',flashvars,params);
//		CKobject.embed('ckplayer/ckplayer.swf', obj.id, 'ckplayer_' + obj.id, '100%', '100%', false, flashvars, params);
