...
var CFCMXK = function () {
	...
	_createClass(CFCMXK, [{
		...
		value: function initVal(opt) {
			...
			_extends(this.cfg, defaultConfig, opt);
			this.initcommonVal();
		}
	}, {
		key: 'initcommonVal',
		value: function initcommonVal() {
			var cfg = this.cfg;
			//初始化大小计算16/9
		...
		key: 'setStyle',
		value: function setStyle(obj, type, val) {
			obj.style[type] = val;
		}
	}, {
		key: 'setAttr',
		value: function setAttr(obj, type, val) {
			obj.setAttribute(type, val);
		}
	}, {
		key: 'platformcheck',
		value: function platformcheck() {
   ...
   * */
		...
		value: function keycode() {
			...
			document.onkeydown = function (e) {
				...
				if (ev.keyCode == "122") {
					//f11 /esc|| ev.keyCode == "27"
					if (cfg.lock) {
						//锁定时间标识，如果时间锁定，一秒后才可以生效
						if (!cfg.f11count) {
   ...
   */
		...
		key: 'resizeplayer',
		value: function resizeplayer(_this) {
			var cfg = _this.cfg;
			//		cfg.flashplayer = [];
   ...
   */
		...
		key: 'videoopt',
		value: function videoopt() {
			// ckobj.changeVolume(0);
			var _this = this;
			var cfg = this.cfg;
			cfg.fullscreenObj.onclick = function () {
				...
				var flag = this.getAttribute('class');
				if (flag == 'fullscreen') {
					cfg.fullscreen = true;
   ...
   */
		...
		key: 'appendvideo',
		value: function appendvideo(html, num) {
			var cfg = this.cfg;
			cfg.flashinfo = [];
			for (var i = 0; i < num; i++) {
   ...
   * */
		...
		key: 'resetflashinfo',
		value: function resetflashinfo(urlarr, titlearr, page, screens, channelinfo, vue) {
			var cfg = this.cfg;
			cfg.flashinfo = [];
			var temp = [];
			var tempchannel = [];
			...
			this.domDeal();
			this.countcreatevideo(cfg.channelnum);
			for (var i = (page - 1) * screens; i < (page - 1) * screens + screens; i++) {
				cfg.flashinfo.push({
   ...
   */
		...
		key: 'createvideo',
		value: function createvideo(num) {
			var html = '';
			switch (num) {
				case 1:
				case 2:
   ...
   */
		...
		key: 'countcreatevideo',
		value: function countcreatevideo(num) {
			var cfg = this.cfg;
			switch (num) {
				case 1:
				...
				case 3:
					...
					var hei = (cfg.WH.hei - thishei * 2) / 2;
					for (var i = 0; i < num; i++) {
						if (i == 0) {
		...
		key: 'muteevent',
		value: function muteevent(num) {
			var cfg = this.cfg;
			var obj = CKobject.getObjectById('ckplayer_' + num);
		...
		key: 'channelevent',
		value: function channelevent(num, channelarr, vue) {
			if (channelarr) {
   ...
   */
		...
		key: 'getStyle',
		value: function getStyle(el, name) {
   ...
   */
		...
		key: 'idstyle',
		value: function idstyle(wid, hei) {
			var obj = {},
			    temp = hei;
			//以宽度为依据,计算比例
			if (wid * 0.5625 > hei) {
   ...
   */
		...
		key: 'FullScreen',
		value: function FullScreen(element) {
   ...
   */
		...
		key: 'setckplayer',
		value: function setckplayer() {
			var cfg = this.cfg;
			var _this = this;
			for (var i = 0; i < cfg.channelnum; i++) {
				(function (i) {
   ...
   */
		...
		key: 'ckplayer',
		value: function ckplayer(obj) {
			var callback = 'xk_loadedHandler' + obj.num;
			var flashvars = {
				f: obj.url, //视频地址
